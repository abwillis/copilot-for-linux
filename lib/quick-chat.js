...
input.addEventListener('keydown', e => {
  ...
  function createQuickChatWindow() {
    ...
    const win = new BrowserWindow({
      ...
      webPreferences: {
        ...
        partition: (typeof getCopilotPartition === 'function') ? getCopilotPartition() : undefined,
    ...
    win.loadURL((typeof getCopilotUrl === 'function') ? getCopilotUrl() : '');
  ...
  async function buildSelectionEnvelope(sourceWin, opts) {
    ...
    const role = src.__copilotRole || (src === getMain() ? 'main' : 'unknown');
  ...
  function registerIpcHandlers() {
    ...
    ipcMain.on(IPC.SEND_SELECTION, async (event, opts) => {
      ...
      const source = (sender && sender.__copilotRole === 'main') ? sender : getMain();
