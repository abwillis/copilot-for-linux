...
// Return true if this submenu already contains a Copilot-managed item.
...
function appendEditItems(editSubmenu) {
  const template = [
    ...
    { type: 'separator' },
    {
      id: 'copilot-edit-select-chat-pane',
...
function appendHelpItems(helpSubmenu) {
  if (submenuHasManagedItem(helpSubmenu, { id: 'copilot-help-application-help', label: 'Application Help' })) {
    return;
  }

  const template = [
    new MenuItem({
      id: 'copilot-help-application-help',
...
function appendSessionItems(sessionSubmenu) {
  if (submenuHasManagedItem(sessionSubmenu, { id: 'copilot-session-reload', label: 'Reload Copilot' })) {
    return;
  }

  const template = [
    new MenuItem({
      id: 'copilot-session-reload',
      label: 'Reload Copilot',
      accelerator: 'Ctrl+R',
      click: () => reloadCopilot({ ignoreCache: false })
    }),
    new MenuItem({
      id: 'copilot-session-hard-reload',
      label: 'Hard Reload',
    ...
    new MenuItem({ type: 'separator' }),
    new MenuItem({
      id: 'copilot-session-clear-cache',
    ...
    new MenuItem({
      id: 'copilot-session-clear-cookies-sign-out',
    ...
    new MenuItem({
      id: 'copilot-session-copy-current-url',
      label: 'Copy Current URL',
      click: () => copyCurrentUrl()
    }),
    new MenuItem({
      id: 'copilot-session-open-current-url-external',
...
function appendFileItems(fileSubmenu, win) {
  if (submenuHasManagedItem(fileSubmenu, { id: 'copilot-file-save-chat-pane', label: 'Save Chat Pane' })) {
    return;
  }
  ...
  const items = [
    new MenuItem({ type: 'separator' }),
    new MenuItem({
      id: 'copilot-file-save-chat-pane',
