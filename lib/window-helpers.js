...
function createWindowHelpers(deps = {}) {
  ...
  function attachCSSAndLayoutHandlers(win, { role = 'window', revealOnReady = true, didStopLoadingHandler = onDidStopLoading } = {}) {
    ...
    win.once('ready-to-show', () => {
      if (revealOnReady) reveal(win);
      // Seed the app-configured CSS width variable if it is not already set,
      // then install the responsive resize listener.  Both delegate to the
      // renderer agent via lib/layout-css.js.  Order matters:  seedTargetVW
      // no-ops when the variable is already present, so seeding first keeps
      // the responsive listener authoritative for subsequent updates.
      if (typeof applyDynamicWidth === 'function') {
        try { applyDynamicWidth(win); }
        catch (e) { console.error(`applyDynamicWidth (${role}) failed:`, e); }
      }
      try { attachVWResize(win); }
      catch (e) { console.error(`attachVWResize (${role}) failed:`, e); }

      // Tag the composer wrapper chain so section 7 of the layout CSS has an
      // attribute to match. Failure here only costs composer width; the
      // transcript rules are independent of it.
      if (typeof attachComposerTracking === 'function') {
        try { attachComposerTracking(win); }
        catch (e) { console.error(`attachComposerTracking (${role}) failed:`, e); }
      }
    });
  }
