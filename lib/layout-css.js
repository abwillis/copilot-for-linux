...
// not. It is widened with the same CLAMP.
//
// DISCOVERY NOW HAPPENS IN JS, NOT CSS.
// This section previously located the composer with a live :has() ladder:
//
//   body :is(div, form, section)
//       :not(:has(CONVO_SCOPE)) :not(:has(nav, header, ...))
//       :has([contenteditable="true"])
//
// Structurally correct, but three descendant searches per candidate element,
// re-evaluated by Chromium on every DOM mutation -- and the composer mutates
// on every keystroke.
//
// renderer/agent.js now performs the identical structural walk once (see
// findComposerWrapperChain: editor -> outward, stop at the app shell) and tags
// the wrapper chain with data-expanded-composer="1". The guards are unchanged
// in substance -- transcript-containing wrappers and nav/header landmarks are
// still excluded, and only multiline editors (contenteditable / textarea /
// role=textbox, never a bare <input>) are considered -- so this remains free of
// app-specific class names. Only the mechanism changed: a plain attribute match
// instead of a live :has() evaluation.
const COMPOSER_MARKER_ATTR = 'data-expanded-composer';
const COMPOSER_MARKER_SELECTOR = `[${COMPOSER_MARKER_ATTR}="1"]`;
...
function buildMaxLayoutCSS({ specificMessageId } = {}) {
    ...
    // Composer wrappers (tagged by renderer/agent.js startComposerTracking),
    // and the editors inside them.
    const COMPOSER_LADDER = COMPOSER_MARKER_SELECTOR;
...
/*                                                                            */
/* The marker is applied by renderer/agent.js, which excludes the app shell    */
/* structurally before tagging, so this cannot widen the page frame. On the    */
/* empty new-chat screen the agent still finds the composer's own wrapper,     */
/* which is the intended target there too. If the agent never runs, nothing    */
/* matches and the composer keeps its default width -- the transcript rules    */
...
function createLayoutCSS(deps = {}) {
    ...
    // Install the composer marker tracker. Re-armed on SPA navigation because
    // the agent's MutationObserver is torn down with the document. Idempotent:
    // startComposerTracking() disconnects any previous observer first.
