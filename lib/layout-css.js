...
// unrendered subtrees entirely.  We inject a CSS + JS override only while
// the Find modal is open, then remove it on close so the lazy-render
...
// ---------------------------------------------------------------------------
// JS-based force-visibility: overrides inline styles + MutationObserver
// to fight the app's virtualizer re-applying content-visibility:auto.
// ---------------------------------------------------------------------------
function buildForceVisibilityScript() {
    ...
    (async function () {
        try {
            ...
            for (var i = 0; i < selectors.length; i++) {
                chatRoot = document.querySelector(selectors[i]);
                if (chatRoot) break;
            }
            var overridden = 0;

            function forceVis(el) {
                try {
            ...
            scope.querySelectorAll('*').forEach(forceVis);

            // ---- Scroll-to-render: force lazy / virtualized content ----
            ...
            if (!window.__copilot_findVisScrollDone && chatRoot) {
                window.__copilot_findVisScrollDone = true;
                var sp = getScrollParent(chatRoot);
                var savedTop = sp.scrollTop;

                // Briefly expand to force browser to lay out all content
                ...
                void sp.offsetHeight; // sync reflow

                // Wait two frames so the browser paints
                await new Promise(function(r) {
                    requestAnimationFrame(function() { requestAnimationFrame(r); });
                });

                // Scroll through to trigger IntersectionObserver-based loaders
                var total = sp.scrollHeight;
                var view  = sp.clientHeight || 500;
                for (var pos = 0; pos <= total; pos += Math.max(view * 0.75, 200)) {
                    sp.scrollTop = pos;
                ...
                // Re-walk after expansion (new nodes may have appeared)
                scope.querySelectorAll('*').forEach(forceVis);
            ...
            // ---- MutationObserver ----
            if (window.__copilot_findVisObs) {
                try { window.__copilot_findVisObs.disconnect(); } catch (e) {}
            }
            var obs = new MutationObserver(function (muts) {
                for (var m = 0; m < muts.length; m++) {
                    var mut = muts[m];
                    if (mut.type === 'attributes') forceVis(mut.target);
                    if (mut.type === 'childList') {
                        var added = mut.addedNodes;
                        for (var n = 0; n < added.length; n++) {
                            if (added[n].nodeType !== 1) continue;
                            forceVis(added[n]);
                            try { added[n].querySelectorAll('*').forEach(forceVis); } catch (e) {}
                        }
                    }
                }
            });
            ...
            window.__copilot_findVisObs = obs;

            // ---- Periodic re-check (catches deferred re-hides) ----
            if (window.__copilot_findVisInterval) clearInterval(window.__copilot_findVisInterval);
            window.__copilot_findVisInterval = setInterval(function() {
                try {
                    (chatRoot || document.body).querySelectorAll('*').forEach(forceVis);
                } catch (e) {}
            }, 2000);

            return { ok: true, overridden: overridden };
        } catch (e) {
...
function buildRestoreVisibilityScript() {
    return `
    (function () {
        try {
            if (window.__copilot_findVisObs) {
                window.__copilot_findVisObs.disconnect();
                delete window.__copilot_findVisObs;
            }
            if (window.__copilot_findVisInterval) {
                clearInterval(window.__copilot_findVisInterval);
                delete window.__copilot_findVisInterval;
            }
            delete window.__copilot_findVisScrollDone;
            document.querySelectorAll('[style]').forEach(function (el) {
                try {
                    if (el.style.getPropertyValue('content-visibility') === 'visible' &&
...
function enableFindContentVisibility(win) {
    ...
    const wc = win.webContents;
    const isAlreadyActive = (findCVTargetWC === wc);

    if (!isAlreadyActive) {
        disableFindContentVisibility();
        findCVTargetWC = wc;

        // --- Layer 1: CSS (inserted once per session) ---
        const css = buildFindContentVisibilityCSS();
        wc.insertCSS(css)
            .then(key => { findCVKey = key; })
            .catch(err => {
    ...
    // --- Layer 2: JS force-visibility (re-runs every call) ---
    const jsScript = buildForceVisibilityScript();
    wc.executeJavaScript(jsScript).catch(err => {
        console.error('enableFindContentVisibility JS override failed:', err);
    });
    try {
        const frames = wc.mainFrame?.framesInSubtree ?? [];
        for (const f of frames) {
            try { f.executeJavaScript(jsScript).catch(() => {}); } catch {}
        }
    } catch {}
}

function disableFindContentVisibility() {
    if (!findCVTargetWC) return;
    const key = findCVKey;
    ...
    // Remove JS overrides + disconnect MutationObserver
    const restoreScript = buildRestoreVisibilityScript();
    try { wc.executeJavaScript(restoreScript).catch(() => {}); } catch {}
    try {
        const frames = wc.mainFrame?.framesInSubtree ?? [];
        for (const f of frames) {
            try { f.executeJavaScript(restoreScript).catch(() => {}); } catch {}
        }
    } catch {}
}
...
module.exports = {
    ...
    buildForceVisibilityScript,
    buildRestoreVisibilityScript,
