...
async function materializeExternalImageAssets(win, html, mdFilePath) {
  ...
  async function saveChatPaneAsNativePDF(win, filePath) {
    ...
    try {
        ...
        // Two passes: one synchronous, then a yield to let SPA handlers
        // flush controlled regions, then a second pass for any newly
        // surfaced collapsibles inside what we just opened.
        if (exportMarkerApplied) {
          try {
            await callRA(win, 'expandForPrint');
            // Let the SPA paint/settle before the second pass.
            await new Promise(r => setTimeout(r, 80));
            await callRA(win, 'expandForPrint');
          } catch (e) {
      ...
      // Hydrate virtualized content. The Fluent virtualizer that hosts the
      // chat only renders DOM for items near the viewport; without this
      // pass, bottom-of-thread messages can be missing from the PDF if
      // the user was scrolled up at export time.
      try {
        ...
        try { console.log('[export-pdf-native] hydrateVirtualizer:', hydrateRes); } catch {}
      } catch (e) {
        console.warn('[export-pdf-native] hydrateVirtualizer failed:', e);
      }
