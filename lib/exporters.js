...
async function materializeInlineImageAssets(win, html) {
  ...
  async function saveSelectionAsPDF(win, filePath) {
    if (!win) return;
    try {
      // For selection PDF, select the content first then print natively.
      const selResult = await selectChatPane(win);
      if (!selResult?.ok) {
        safeShowError('Export Selection as PDF', 'No selection or chat pane found.');
        return;
      }
...
async function saveChatPaneAsNativePDF(win, filePath) {
    ...
    try {
      const snapshot = await getChatPaneSnapshot(win);
      const selectorGroup = snapshot?.selector
      ...
      const prepScript = `
        (function() {
          try {
            var selectorGroup = ${JSON.stringify(selectorGroup)};
            // Find the actual chat pane element
            var chatPane = document.querySelector(selectorGroup);
