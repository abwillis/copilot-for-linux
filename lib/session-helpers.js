...
function createSessionHelpers(deps = {}) {
  ...
  function _partition() {
    return (typeof getCopilotPartition === 'function') ? getCopilotPartition() : '';
  }

  function _copilotUrl() {
    return (typeof getCopilotUrl === 'function') ? getCopilotUrl() : '';
  }
  ...
  async function _ensureConfigFilePath() {
  ...
  async function openConfigFile() {
    const configPath = await _ensureConfigFilePath();
  ...
  function showAboutDialog() {
    try {
      ...
      const parent = getActiveCopilotWindow();
